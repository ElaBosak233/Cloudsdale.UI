  ...
  <v-dialog v-model="dialog" min-width="50vh" max-width="50vh">
    <v-card class="pa-5">
      <div class="d-flex justify-between align-center">
        <v-icon icon="mdi-image-edit" size="35" color="primary"></v-icon>
      ...
      <v-sheet class="d-flex justify-space-between mt-5">
        <div class="d-flex justify-center align-center pa-3">
          <IAvatar :size="80" :user-id="item.id" />
        </div>
...
async function deleteAttachment() {
  ...
  const { data: res } = await useAuthFetch(`/media/users/avatar/${props.item.id}`, {
    method: "DELETE",
  });
