  ...
  <v-dialog v-model="dialog" width="50vh">
    <v-card class="pa-5">
      <div class="d-flex justify-between align-center">
        <v-icon icon="mdi-account-multiple-minus" size="35" color="error"></v-icon>
        <v-chip :label="true" class="mx-2" color="error">
          <span class="font-weight-bold text-h6">团队删除</span>
        </v-chip>
      </div>
      <div class="d-flex justify-center pt-5">
        <span class="text-md-body-1"
          >团队将被永久删除，是否继续？<span class="font-italic font-weight-bold"
            >（真的很久！）</span
          ></span
        >
      </div>
      <div class="d-flex justify-end pt-5">
        <v-btn variant="flat" color="error" class="mx-1" @click="dialog = false"
          >取消</v-btn
        >
        <v-btn variant="flat" color="primary" class="mx-1" @click="deleteTeam(item?.id)"
          >确定</v-btn
        >
      </div>
    </v-card>
...
async function deleteTeam(id: number) {
  ...
  if (resObj?.code === 200) {
    snackBarStore.showSnackbar("团队删除成功", "success");
    emit("finished");
  }
}
</script>
