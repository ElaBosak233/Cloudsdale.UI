<template>
  <v-list-item base-color="teal" @click="openDialog()">
    ...
    <v-tooltip activator="parent" location="top center"> 头像管理 </v-tooltip>
  </v-list-item>
    ...
    <v-card class="pa-5">
      ...
      <v-sheet class="d-flex justify-space-between mt-5">
        <div class="d-flex justify-center align-center pa-3">
          <TAvatar :size="80" :team-id="item.id" />
        </div>
...
async function deleteAttachment() {
  ...
  const { data: res } = await useAuthFetch(`/media/teams/avatar/${props.item.id}`, {
    method: "DELETE",
  });
  const resObj = res.value as Response;
  if (resObj.code === 200) {
    snackBarStore.showSnackbar("头像清除成功", "success");
    teamStore.deleteTeamAvatar(props.item.id);
  } else {
...
async function uploadAttachment() {
  ...
  if (files.value[0]) {
    ...
    if (resObj.code === 200) {
      ...
      teamStore.updateTeamAvatar(props.item.id);
    } else {
