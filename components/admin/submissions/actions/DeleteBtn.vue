  ...
  <v-dialog v-model="dialog" width="50vh">
    <v-card class="pa-5">
      <div class="d-flex justify-between align-center">
        <v-icon icon="mdi-alert-decagram" size="35" color="error"></v-icon>
        <v-chip :label="true" class="mx-2" color="error">
          <span class="font-weight-bold text-h6">提交记录删除</span>
        </v-chip>
      </div>
      <div class="d-flex justify-center pt-5">
        <span class="text-md-body-1"
          >提交将被永久删除，是否继续？<span class="font-italic font-weight-bold"
            >（真的很久！）</span
          ></span
        >
      </div>
...
async function deleteSubmission(id: number) {
  ...
  if (resObj?.code === 200) {
    snackBarStore.showSnackbar("提交记录删除成功", "success");
    emit("finished");
  }
}
</script>
